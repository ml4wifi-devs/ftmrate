  ...
  if (mobilityModel == "Distance")
    {
      ...
      // Place second node at (2*distance, 0)
      Ptr<MobilityModel> mobilitySta = wifiStaNodes.Get (1)->GetObject<MobilityModel> ();
      mobilitySta->SetPosition (Vector3D (2*distance, 0., 0.));
    }
  else if (mobilityModel == "RWPM")
    {
      // Place AP at (0, 0)
  ...
  if (!pcapName.empty ())
    {
      ...
      phy.EnablePcap (pcapName, staDevice);
    }
